...
import javax.swing.*;
import java.sql.*;
public class Epin extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b1) {
            try {
                ...
                int p=Integer.parseInt(accno);
                Class.forName("com.mysql.cj.jdbc.Driver");
                Connection conn = null;
                conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/bank", "root", "");
                Statement s = conn.createStatement();
                String s6="select pin from employee where eid="+p+"";
...
npin=" "+npin;
                String s8="UPDATE employee SET pin="+npin+" where  eid="+p+"";
                s.execute(s8);

                JOptionPane.showMessageDialog(null, "PIN changed successfully");
                setVisible(false);
                new Empfront(accno).setVisible(true);
            }
            catch(Exception e){
                e.printStackTrace();
            }
