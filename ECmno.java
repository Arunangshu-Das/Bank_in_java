...
import javax.swing.*;
import java.sql.*;
public class ECmno extends JFrame implements ActionListener {
    JTextField tf1,tf2,tf3;
    JLabel l1,l2,l3,l4;
    JButton b1,b2;
    String who,accno;
    ECmno(String who,String accno)
    {
        ...
        l2 = new JLabel("New Mobile Number:");
        l2.setFont(new Font("System", Font.BOLD, 16));
        l2.setForeground(Color.BLACK);

        l3 = new JLabel("Re-Enter New Mobile Number:");
        l3.setFont(new Font("System", Font.BOLD, 16));
        l3.setForeground(Color.BLACK);

        l4 = new JLabel("Change your Mobile Number");
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b1) {
            try {
                ...
                String s6="select mno from customer where accno="+accno+"";
                ResultSet rs = s.executeQuery(s6);
                while (rs.next()){
                    s2=rs.getString("mno");
                }
                if (!npin.equals(rpin)) {
                    JOptionPane.showMessageDialog(null, "Entered Mobile Number does not match");
                    return;
                }
                ...
                String s8="UPDATE customer SET mno="+npin+" where  accno="+accno+"";
                s.execute(s8);
